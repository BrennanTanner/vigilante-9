...
@export var use_paths := true
@export var path_start_point: Vector2 = Vector2.ZERO
@export var path_end_point: Vector2 = Vector2(500, 500)
@export var path_segment_count: int = 100
@export var path_width: float = 5.0
@export var path_flatten_radius: float = 3.0
@export var path_curve: Curve
...
var flattened_positions: Dictionary = {}
...
func _ready():
	if generate_terrain and not Engine.is_editor_hint():
		...
		if use_paths:
			var path_points = generate_path_points()
			flatten_terrain_around_path(path_points)
...
func generate_bigmesh(chunk: Vector2i):
	...
	for x_division in bigmesh_subdivision:
		...
		for z_division in bigmesh_subdivision:
			...
			var flat_y = flattened_positions.get(Vector2(coord_3d.x, coord_3d.z), null)
			if flat_y != null:
				coord_3d.y = flat_y
...
func generate_path_points() -> Array:
	var path_points = []
	var total_distance = path_end_point.distance_to(path_start_point)
	var direction = (path_end_point - path_start_point).normalized()

	for i in range(path_segment_count):
		var t = float(i) / (path_segment_count - 1)
		var straight_point = path_start_point.lerp(path_end_point, t)

		# Add noise-based offset perpendicular to direction
		var offset_dir = Vector2(-direction.y, direction.x)
		var noise_offset = terrain_noise.get_noise_2dv(straight_point * 0.01)
		var offset = offset_dir * noise_offset * path_width

		var final_point = straight_point + offset
		path_points.append(final_point)

	return path_points

func flatten_terrain_around_path(path_points: Array):
	for point in path_points:
		# Get height at the center of the path
		var center_y = get_terrain_height(point.x, point.y)

		# Sample terrain within radius and flatten
		var resolution = 5 # number of samples across width
		for i in resolution:
			var angle = (float(i) / resolution) * TAU
			var offset = Vector2(cos(angle), sin(angle)) * path_flatten_radius
			var sample_pos = point + offset

			# Modify terrain here or store data to be applied later
			flatten_single_point(sample_pos, center_y)

func flatten_single_point(pos: Vector2, target_y: float):
	flattened_positions[pos] = target_y
